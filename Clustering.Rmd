...
emp_churn <- read.csv("C:/Users/19737/Downloads/emp_churn.csv")
```
...
ggscatter(emp_churn, x = "PerformanceRating", y = "PercentSalaryHike",
          add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE, cor.method = "pearson",xlab = "Performance rating", ylab = "Salary hike in percentage")
```
```{r}
...
```
```{r}
emp_churn$Attrition <- as.numeric(emp_churn$Attrition) - 1
str(emp_churn)
```
```{r}
ndata <- emp_churn %>% filter(Attrition != 0)
ndata
```
```{r}
library(tidyverse)
cdata=ndata %>% select(MonthlyIncome, ï..Age)
cdata
```
```{r}
library(factoextra)
...
```
```{r}
kCluster <- kmeans(cdata, 4, iter.max = 20, nstart = 20)
kCluster
```
```{r}
table(kCluster$cluster, cdata$ï..Age)
```
```{r}
plot(cdata[c("MonthlyIncome", "ï..Age")], col = kCluster$cluster)
```
```{r}
# As we can see from abvove graphs, that younger people have comparitively lower income than people with older ages. The lurking variable could be industry experience.
```
